...
class TestRulesExport:
    ...
    def test_export_all_rules(self):
        ...
        assert response.status_code == 200
        assert "yaml" in response.headers.get("content-type", "").lower() or response.text.startswith("presets:")
...
class TestWebSocket:
    ...
    def test_websocket_endpoint_exists(self):
        ...
        import websocket
        try:
            ws = websocket.create_connection("ws://127.0.0.1:8000/ws/progress", timeout=5)
            ws.close()
            connected = True
        except:
            connected = False
