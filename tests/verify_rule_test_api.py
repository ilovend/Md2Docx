...
| Val1 | Val2 |
    ...
    config = {
        "rules": {
            ...
            "table_border": {"enabled": True, "parameters": {"border_size": 8, "border_color": "00FF00"}}
        }
    }
    
    try:
        response = requests.post(f"{BASE_URL}/rules/test", json={
            "markdown": markdown,
            "config": config
        })
        ...
        if response.status_code == 200:
            ...
            if "<html>" in html or "<div" in html: # Mammoth or similar often returns just div
